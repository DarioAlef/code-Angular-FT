...
def generate_instruction_fallback(file_path: str, component_code: str) -> str:
    ...
    if "MatPaginator" in component_code:
        return f"Crie um componente Angular para listar {title.lower()} com paginação e búsca"
...
### 3.3 Como Usar o Script de Geração
...
**Passo 2: Obtenha chave da API Gemini**
- Visite: https://makersuite.google.com/app/apikeys
- Crie uma nova chave
- Copie e configure:
```bash
...
**Output esperado:**
```
📂 Encontrados 47 componentes
⚙️  Limitando a 10 primeiros componentes para teste
[1/10] Processando: skeleton-web/src/app/components/base.component.ts
  ✅ Código extraído (4200 caracteres)
  📝 Instrução: Crie um componente base abstrato para servir como...

[2/10] Processando: skeleton-web/src/app/components/app/app.component.ts
  ...

✅ Dataset salvo em: dados_2026.jsonl
   Total de exemplos: 10
```

**Arquivo gerado: `dados_2026.jsonl`**
```json
{"instruction": "Crie um componente base abstrato para...", "response": "export abstract class BaseComponent<T>..."}
{"instruction": "Crie um componente raiz para a...", "response": "export class AppComponent..."}
...
```

### 3.4 Expandindo o Dataset

Para gerar todos os 47+ componentes (não apenas 10):

```bash
# Edite a última linha de 1_generate_dataset.py:
# dataset = process_components(SKELETON_WEB_DIR, gemini_model, max_samples=None)
#                                                                      ^^^^
# Mude para None para processar TODOS
...
## 🖥️ PARTE 4: Setup do Ambiente de Treinamento

### 4.1 Opções de Ambiente

| Ambiente | GPU | RAM | Custo | Recomendado |
|----------|-----|-----|-------|-------------|
| Google Colab (gratuito) | T4/A100 | 12GB | $0 | ✅ SIM |
| Google Colab Pro | A100 | 48GB | $10/mês | ✅ Melhor |
| DGX (se empresa tem) | H100 | 80GB | Interno | ✅ Ótimo |
| Seu Laptop | CPU | 16GB | $0 | ❌ Muito lento |

### 4.2 Setup Google Colab (Recomendado)

**Vantagens:**
- GPU gratuita (T4 ou A100)
- Já tem Python, CUDA, PyTorch
- Fácil salvar em Google Drive

**Passo 1: Crie notebook em Google Colab**
- Visite: https://colab.research.google.com/
- Arquivo → Novo notebook → Nomeie como `QLoRA_Training.ipynb`

**Passo 2: Na primeira célula, instale dependências**
